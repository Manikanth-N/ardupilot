...
#define AP_MOUNT_Gimbal_Camera_RESEND_MS  1000     // resend angle targets to gimbal at least once per second
#define AP_MOUNT_Gimbal_Camera_SEARCH_MS  60000    // search for gimbal for 1 minute after startup
#define AP_MOUNT_Gimbal_Camera_ATTITUDE_INTERVAL_US    20000  // send ATTITUDE and AUTOPILOT_STATE_FOR_GIMBAL_DEVICE at 50hz
#define AP_MOUNT_Gimbal_Camera_RESEND_CAPTURE_MS 500 // resend capture command to gimbal-camera 


// update mount position
void AP_Mount_Gimbal_Camera::update()
{   
    ...
    // get camera commands on RC input
    get_cam_operations_on_rc_input();

}

// return true if healthy
bool AP_Mount_Gimbal_Camera::healthy() const
{
...
bool AP_Mount_Gimbal_Camera::get_attitude_quaternion(Quaternion& att_quat)
{
...
void AP_Mount_Gimbal_Camera::find_gimbal()
{
    ...
    if ((now_ms > AP_MOUNT_Gimbal_Camera_SEARCH_MS) && hal.util->get_soft_armed()) {
        return;
    }
...
void AP_Mount_Gimbal_Camera::handle_gimbal_device_information(const mavlink_message_t &msg)
{
...
void AP_Mount_Gimbal_Camera::handle_gimbal_device_attitude_status(const mavlink_message_t &msg)
{
    // exit immediately if this is not our message
    if (msg.sysid != _sysid || msg.compid != _compid) {
        return;
...
void AP_Mount_Gimbal_Camera::request_gimbal_device_information() const
{
    if (_link == nullptr) {
        return;
    }
...
bool AP_Mount_Gimbal_Camera::start_sending_attitude_to_gimbal()
{
    ...
    const MAV_RESULT res = _link->set_message_interval(MAVLINK_MSG_ID_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE, AP_MOUNT_Gimbal_Camera_ATTITUDE_INTERVAL_US);
...
void AP_Mount_Gimbal_Camera::send_gimbal_device_retract() const
{
...
void AP_Mount_Gimbal_Camera::send_gimbal_device_set_rate(float roll_rads, float pitch_rads, float yaw_rads, bool earth_frame) const
{
    // prepare flags
...
void AP_Mount_Gimbal_Camera::send_gimbal_device_set_attitude(float roll_rad, float pitch_rad, float yaw_rad, bool earth_frame) const
{
    // exit immediately if not initialised
    if (!_initialised) {
        return;
...
// get camera operations on RC input
void AP_Mount_Gimbal_Camera::get_cam_operations_on_rc_input()
{
    // exit immediately if no RC input
    if (!rc().has_valid_input()) {
        return;
    }
    // GCS_SEND_TEXT(MAV_SEVERITY_DEBUG,"after rc validation");
    // Find the Rc_channel Assigned to respective operations
    const RC_Channel *camera_trigger_ch = rc().find_channel_for_option(_instance == 0 ? RC_Channel::AUX_FUNC::CAMERA_TRIGGER : RC_Channel::AUX_FUNC::CAMERA_TRIGGER);
    const RC_Channel *camera_rec_video_ch = rc().find_channel_for_option(_instance == 0 ? RC_Channel::AUX_FUNC::CAMERA_REC_VIDEO : RC_Channel::AUX_FUNC::CAMERA_REC_VIDEO);
    const RC_Channel *camera_zoom_ch = rc().find_channel_for_option(_instance == 0 ? RC_Channel::AUX_FUNC::CAMERA_ZOOM : RC_Channel::AUX_FUNC::CAMERA_ZOOM);

    // get rc input
    const int16_t camera_trigger_in = (camera_trigger_ch == nullptr) ? 0 : camera_trigger_ch->get_radio_in();
    const int16_t camera_rec_video_in = (camera_rec_video_ch == nullptr) ? 0 : camera_rec_video_ch->get_radio_in();
    const int16_t camera_zoom_in = (camera_zoom_ch == nullptr) ? 0 : camera_zoom_ch->get_radio_in();

    // Camera Triggering
    // GCS_SEND_TEXT(MAV_SEVERITY_DEBUG," Calling Camera Trigger");
    camera_capture(camera_trigger_in);

    // Camera Recording
    camera_recording(camera_rec_video_in);

    // Camera Zoom
    camera_zoom(camera_zoom_in);
...
// Camera Trigger
void AP_Mount_Gimbal_Camera::camera_capture(int16_t camera_trigger_in)
{
    if (camera_trigger_in > PWM_HIGH) 
    {
        if (AP_HAL::millis() - trigger_start_time >= AP_MOUNT_Gimbal_Camera_RESEND_CAPTURE_MS)
        {
            // Camera trigger is active
            resend_capture_now = true;
            trigger_start_time = AP_HAL::millis();
        }
    } 
    else 
    {
        // Camera trigger is inactive
        resend_capture_now = false;
    }

    // send Capture command
    if (resend_capture_now)
    {
        mavlink_msg_command_long_send(_chan,
                                  ...
                                  0,
                                  1,
                                  1, 0, 0,  // param4 ~ param6 unused
                                  0);
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,"Image Captured....... %d ", _chan);
        resend_capture_now=false;
    }
}

// Camera Recording
void AP_Mount_Gimbal_Camera::camera_recording(int16_t camera_rec_video_in)
{
    // Video start and stop
    if (camera_rec_video_in > PWM_HIGH || camera_rec_video_in > PWM_LOW) 
    {
        current_video_state=VIDEO_START;
    } 
    else if (camera_rec_video_in < PWM_LOW) 
    {
        current_video_state=VIDEO_STOP;
    }

    // send camera recording commands
    if (current_video_state != previous_video_state) 
    {
        previous_video_state = current_video_state;        
        if (current_video_state == VIDEO_START)
        {
            mavlink_msg_command_long_send(_chan,
                                    _sysid,
                                    _compid,
                                    ...
                                    0);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO,"Video Recording Started.......");
        ...
        else
        {
            mavlink_msg_command_long_send(_chan,
                                    ...
                                    0, 0, 0,  // param4 ~ param6 unused
                                    0);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO,"Video Recording Stopped.......");
        }
    }
}

void AP_Mount_Gimbal_Camera::camera_zoom(int16_t camera_zoom_in)
{
    if (camera_zoom_in > PWM_HIGH) 
    {
        current_Zoom_state=ZOOM_IN;
    }
    else if (camera_zoom_in < PWM_LOW) 
    {
        current_Zoom_state=ZOOM_OUT;
    }
    else if (camera_zoom_in > PWM_LOW && camera_zoom_in < PWM_HIGH)
    {
        current_Zoom_state=ZOOM_NOTHING;
    }

    // send camera zoom commands
    // send zooming
    if (current_Zoom_state != previous_zoom_state)
    {
        previous_zoom_state = current_Zoom_state;
        if(current_Zoom_state==ZOOM_IN)
        {
            mavlink_msg_command_long_send(_chan,
                                    ...
                                    0, 0, 0,  // param4 ~ param6 unused
                                    0);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO,"Zoom In .......");
        }
        else if (current_Zoom_state==ZOOM_OUT)
        {
            mavlink_msg_command_long_send(_chan,
                                    ...
                                    0, 0, 0,  // param4 ~ param6 unused
                                    0);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO,"Zoom Out .......");
        }
        else if (current_Zoom_state == ZOOM_NOTHING)
        {
            mavlink_msg_command_long_send(_chan,
                                    _sysid,
                                    _compid,
                                    MAV_CMD_SET_CAMERA_ZOOM,
                                    0,        // confirmation of zero means this is the first time this message has been sent
                                    0,          // 0 for Zoom Stopped
                                    0,
                                    0,
                                    0, 0, 0,  // param4 ~ param6 unused
                                    0);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO,"Zoom Stopped.......");
        }
    }
}

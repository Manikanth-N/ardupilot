/*
  Gimbal_Camera mount backend class
 */
#pragma once
...
class AP_Mount_Gimbal_Camera : public AP_Mount_Backend
{
public:
...
private:
    ...
    // 
    void get_cam_operations_on_rc_input();

    //
    void camera_capture(int16_t camera_trigger_in);

    //
    void camera_recording(int16_t camera_rec_video_in);

    //
    void camera_zoom(int16_t camera_zoom_in);
    ...
    // For camera capture
    bool previous_trigger_status = false;
    unsigned long trigger_start_time = 0;
    bool resend_capture_now = false;

    // Max and Min PWM Pulse
    const int16_t PWM_HIGH = 1700;
    const int16_t PWM_LOW = 1300;
    ...
    enum VideoState 
    {
        VIDEO_START,
        VIDEO_STOP
    }current_video_state,previous_video_state = VIDEO_STOP;
    ...
    enum ZoomState 
    {
        ...
        ZOOM_OUT,
        ZOOM_NOTHING
    }current_Zoom_state, previous_zoom_state = ZOOM_NOTHING;
